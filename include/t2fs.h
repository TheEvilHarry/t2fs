...
typedef struct MBR {
	...
	int versaoDisco;
	int tamSetor;
	int BInicialTabela; //Byte inicial da tabela de particoes
	int NumParticoes;
	int SetorInicialParticao1;
	int SetorFinalParticao1;
} t_MBR;
...
#define FILETYPE_INV 0x00
#define FILETYPE_ARQ 0x01
#define FILETYPE_DIR 0x02
...
typedef struct directoryEntry {
    ...
    BYTE    fileType;                   /* Tipo do arquivo: regular (0x01) ou diret�rio (0x02) */
    DWORD   fileSize;                   /* Numero de bytes do arquivo                          */
	DWORD	firstBlock;					/*Primeiro bloco de dados do arquivo/diretório correspondente */
} t_entradaDir;

typedef struct openedfile{

	t_entradaDir* registro;
	long int currentPoint;  //ponteiro da posição atual, em bytes ou numero de entrada no diretório
	DWORD blocoPai;
...
int opened_files;
short int filesMap[MAX_OPENEDF];
